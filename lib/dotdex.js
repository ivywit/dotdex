...
`
  ...
  push: async function () {
    try {
      ...
      for (const index in files) {
        ...
        if (file !== '.git' && keys.indexOf(file) === -1) {
          await git.rm(`${path}/${file}`);
        }
      }
  ...
  pull: async function () {
    try {
      ...
      await git().clone(Config.repo, path);
      if (fs.existsSync(path)) {
          if (fs.existsSync(`${path}/dotdex`)) await Config.load(`${path}/dotdex`, true);
	  ...
	  if (!fs.existsSync(filePath)) await fs.mkdir(filePath, {recursive: true})  
